...
# Same shared, board-agnostic ultima-app source — bind-mounted read-only at a
# fixed container path (see build.sh) and copied into WORKDIR here so the
# build never writes into the shared source tree.
...
# KERNEL_CONFIG_FRAGMENTS one this project already learned the hard way once.
# nostamp forces do_unpack — and everything
...
do_install() {
    ...
    # No kernel-module-tidss RDEPENDS: the display driver isn't packaged as a
    # separate loadable module in this build, so there's nothing to force-load
    # and no such package to depend on. An earlier build did force-load it via
    # /etc/modules-load.d + a kernel-module-tidss RDEPENDS; removing that was
    # required, not just cleanup — do_rootfs fails outright ("nothing provides
    # kernel-module-tidss") once that package stops being produced.

    # Persistent odometer state. This is just the empty mountpoint —
    # wic/ultima-beagley-ai.wks.in provisions the actual /data partition (p3,
