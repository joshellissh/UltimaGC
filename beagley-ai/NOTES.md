...
## mycam004m camera driver ported to BeagleY-AI, folded into the Yocto layer (2026-08-30)

The `mycam004m` driver (separate repo, `~/code/mycam004m` — MYIR MY-CAM004M
quad-AHD decoder, previously only targeting BeaglePlay/AM625 via J17) is now
ported to this board's CSI0 connector and wired into this layer's build.
`mycam004m.c` itself needed zero functional changes — it was already
SoC-generic V4L2 async/fwnode code, confirmed by grepping for board-name
strings before starting (every hit was a comment, since fixed to read as
multi-board). The actual port is new devicetree wiring plus a new Yocto
recipe:

- **`recipes-kernel/linux/linux-bb.org/0002-arm64-dts-k3-am67a-beagley-ai-add-mycam004m-csi0.patch`**
  — camera nodes (main_i2c2 + its pinmux, `camera@30`, `cdns_csi2rx0`/
  `dphy0`/`ti_csi2rx0` enabled), same pattern as the existing USB1 patch.
  **Not a devicetree overlay** — this board boots via Falcon (R5 SPL straight
  to a FIT with the DTB already baked in; no U-Boot proper, no GRUB, nothing
  ever consults `/boot/overlays` or runs `fdt apply`), so a source patch
  rebuilt into the FIT's DTB is the only mechanism that actually ships a DT
  change on this board, same as the USB1 fix already established.
- **`recipes-kernel/mycam004m/mycam004m.bb`** — new recipe, `inherit module`,
  bind-mounts `~/code/mycam004m` (same external-source pattern as
  `ultima-app.bb`; `build.sh` grew a matching `-v ...:/home/builder/yocto/mycam004m-src:ro`
  mount, defaulting to `$HOME/code/mycam004m`). Builds both `mycam004m.ko`
  (real decoder) and `mycam004m-fake.ko` (static-image test backend,
  already hardware-verified on BeaglePlay) — `kernel-module-split` produces
  separate packages for each, both pulled in automatically via `IMAGE_INSTALL
  += "mycam004m"` (`tisdk-base-image.bbappend`) through the umbrella
  package's `RDEPENDS`.
- Both modules force-loaded via `/etc/modules-load.d/mycam004m.conf` (same
  reasoning as `pvrsrvkm.conf`: out-of-tree modules don't coldplug on this
  build). A new `mycam004m-select-backend.service` (ordered the same way as
  `ultima-data-mount.service`/`ultima-app.service` — `DefaultDependencies=no`,
  `WantedBy=local-fs.target`, `Before=ultima-app.service`) runs at boot and
  symlinks `/dev/mycam/cam1..4` to whichever backend `/data/camera-backend`
  names, **defaulting to `fake`** until real MY-CAM004M hardware is cabled to
  CSI0 and verified — `echo real > /data/camera-backend && reboot` (or
  `select-camera-backend.sh real` directly over SSH) switches it.

**What's actually verified, what isn't** (same "verified means something
specific" discipline as the rest of this file): the driver builds clean
(`W=1`, zero warnings) against this board's real `linux-bb.org` 6.12.43 tree;
the new patch applied and the **full production board `.dts`** (not just a
standalone overlay) compiles clean through this tree's own `cpp` +
`scripts/dtc/dtc`, decompiling back to confirm `camera@30` lands under
`i2c@20020000` with its endpoint correctly cross-linked to `cdns_csi2rx0`'s
`port@0`; and — the real end-to-end check — `./build.sh mycam004m` actually
ran through bitbake against this layer for real (not just a manual `make`):
kernel rebuilt with the new patch, `mycam004m` recipe's `do_compile` through
`do_package_qa`/`do_create_spdx` all succeeded, both `kernel-module-mycam004m`
and `kernel-module-mycam004m-fake` `.ipk`s produced, umbrella package's
`RDEPENDS` correctly lists both, all my extra files (scripts, systemd unit,
modules-load.d conf, the fake backend's 4×4.1MB firmware images) landed in
the right package. Zero new warnings (the build's one pre-existing WARNING,
about `ultima-data-mount`/`ultima-splash` bbappends not applying to the R5
multiconfig, predates this work and is unrelated).

**Not verified, and can't be without real hardware:** two open questions,
flagged inline in the patch, the recipe comments, and `mycam004m`'s own
README:
1. Which of J722S's four independent CSI2RX/D-PHY instances CSI0 is
   physically wired to. No schematic confirming it turned up anywhere
   searched (BeagleY-AI's own kernel tree, TI's upstream reference overlays,
   `gh search code --repo beagleboard/linux`, openbeagle.org — blocked by an
   Anubis bot-detection challenge before any content loaded).
   Targets instance 0 as the natural first guess; try instance 1
   (`cdns_csi2rx1`/`dphy1`/`ti_csi2rx1`) next if probe succeeds (dmesg:
   `found N4 decoder: DEV_ID 0xb0`) but no signal ever locks.
2. Whether the decoder is adequately powered without `reset-gpios`/
   `pwren-gpios` — CSI0 appears to expose no spare GPIOs (unlike BeaglePlay's
   J17), so both are omitted. Real precedent this may not matter:
   BeaglePlay's own hardware-verified single-camera capture also ran with
   both unset (the J17 cable in that bring-up didn't carry them either — see
   `mycam004m.c`'s comment near the cold-power-on I2C retry logic).

Next milestone is real hardware: cable a MY-CAM004M to CSI0, flash this
image, `dmesg | grep mycam004m` for the chip-ID readback, then
`docs/testing.md` in the mycam004m repo for the `media-ctl`/`v4l2-ctl`
verification sequence. 4-simultaneous-camera capture (the N4-side
virtual-channel question) is unrelated to this board port and remains open
regardless — single-camera is the milestone here, matching where BeaglePlay's
own bring-up already stood.

### Real camera bench-verified; boot bring-up productized (2026-08-31)

Real MY-CAM004M hardware went on the bench and both open questions above are
answered: **CSI0 is instance 0** (`cdns_csi2rx0`/`dphy0`/`ti_csi2rx0` —
frames DMA through the `30102000.ticsi2rx` capture nodes), and **the
no-reset-/pwren-GPIO powering is adequate** (N4 probes at I2C 0x31 on bus 4
and streams). The one non-obvious runtime requirement, hardware-confirmed:
**`VIDIOC_STREAMON` fails with `EPIPE` until the two SoC-side CSI subdev
formats are set** (`cdns_csi2rx.30101000.csi-bridge` and `30102000.ticsi2rx`
both boot with a default `UYVY8_1X16/640x480` active format that never
auto-propagates from mycam004m's correct pads) — `media-ctl -V ... :0/0
[fmt:UYVY8_1X16/1920x1080 field:none]` on each, see `docs/testing.md` in the
mycam004m repo. Note this board's media-ctl (1.28.1) spells that `-V`, and
this ticsi2rx registers **6** DMA contexts, of which only 0–3 are the four
AHD inputs.
...
- **Real backend is the boot default** — `mycam004m-select-backend.sh` now
  defaults to `real`; `echo fake > /data/camera-backend` flips it back (a
  fresh flash wipes `/data`, so a flashed card boots real with no manual
  step).
- **`fps=25` modprobe options ship in the image**
  (`modprobe.d/mycam004m-options.conf` — the attached camera is AHD 1080p25;
  the driver's fps=30 default only half-locks). Was a live `/etc` edit on the
  bench board.
- **The in-tree CSI modules are force-loaded** alongside the out-of-tree pair
  (`modules-load.d/mycam004m.conf`): their udev coldplug is raceable — the
  first real-backend boot ran the symlink resolve before a single capture
  node existed.
- **New `mycam004m-configure-pipeline.service`** does the real-backend
  bring-up after boot: bounded wait for the four capture nodes (the
  v4l2-async graph registers them *after* modules-load returns), re-runs the
  symlink resolve (node numbers shift between boots — cam1 has been video6
  and video4; only `/dev/mycam/camN` is stable), then sets the two subdev
  formats. Deliberately **not** `Before=ultima-app.service` — the wait must
  never delay first dash frame, and the app only opens cameras when a camera
  screen comes up. The early `mycam004m-select-backend.service` stays cheap
  and now treats a too-early real resolve as non-fatal.
- `media-ctl` (the split v4l-utils package) added to the image via RDEPENDS;
  the driver repo's `select-camera-backend.sh` learned to ignore contexts
  4–5.

**First boots of the productized image (2026-08-31, late): the whole chain
verified end-to-end on hardware.** Flash → power on → N4 probes at 2 s →
CSI modules force-loaded → pipeline service waits, resolves
`/dev/mycam/cam1..4`, sets formats → app opens the CAMERAS screen and
input 0 streams live on the dash with zero manual steps. The three empty
inputs fail `VIDIOC_STREAMON` once per app retry cycle — expected and
harmless (no camera, no configured route; the app keeps retrying, so a
camera that appears later just starts working). Two hardware findings from
the same boots:

- **The recurring N4 I2C dropout was a loose power connector on the
  MY-CAM004M board** — not the FFC, not the BeagleY. One boot logged 212
  `failed reading` NAKs and dead STREAMONs; after reseating the camera
  power connector the next boot was completely clean (0 I2C errors). The
  earlier mid-session dropout on the bench almost certainly had the same
  cause. Boot-order note from these boots: with the CSI modules
  force-loaded, the real ticsi2rx contexts can now enumerate *before*
  mycam004m-fake, so even absolute video-node numbers for the same backend
  differ boot to boot — `/dev/mycam/camN` is the only stable name, which is
