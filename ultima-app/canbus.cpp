...
// signals, beams, axle lift). See docs/MCE18-manual_V3+V4.pdf and
// GAUGE-CLUSTER.md's MCE18 section for the full frame layout. Unit is
// configured via DSS (confirmed 2026-08-21): "CANchecked 0-5000mV" protocol,
// TX Base ID 0x700 (datasheet default), CAN speed 1 Mbit/s to match Syvecs
// CAN2 and can0's udev-configured bitrate. The AIN0-8/DIN0-7/AUX1-3 frame
// layout (this frame + Base ID+2 below) is identical between the
// "0-1023bit" and "0-5000mV" protocol options — only the frequency-channel
// frames at Base ID+3 onward (unused here) differ — so this TX Base ID and
// byte layout aren't protocol-dependent, just the protocol choice below.
static constexpr quint32 kMce18Base = 0x700;
// AIN0-8 are millivolts (0-5000mV) directly under the configured
// "CANchecked 0-5000mV" protocol — the unit does its own ADC-to-voltage
// conversion internally, no raw-counts-to-mV scaling needed on our end.
// Fuel sender: 1V empty, 4V full (per sender spec) — narrower than the AIN's
// own 0-5V range, so this is still a conversion against the sender's own
// linear span, just no longer stacked on top of a raw-to-mV one.
...
void CanBus::decodeFrame(quint32 id, const quint8 *d, int dlc)
{
    ...
    case kMce18Base: {                                   // MCE18 frame 1: AIN0-3
        ...
        // AIN0 arrives as millivolts directly (0-5000mV protocol). Fuel
        // sender is 1V empty / 4V full (linear, per sender spec) — scale
        // against that narrower span.
        double mv = be_u16(d, 0);
