...
- **`CameraGridScreen.qml`** — Persistent swipeable screen (mirror of `DiagnosticScreen`, reached by swiping right off the main dash) showing the same 4 camera feeds as a plain 2x2 grid, one quadrant per physical camera, no stitching — a raw-feed debug view alongside `Camera360Screen`'s stitched one. Separate file/screen rather than a mode of `Camera360Screen` so that screen's opacity-fade behavior stays untouched.
...
- **`SimEngine.qml`** — **Not loaded at runtime.** Bundled only as a reference/potential `--demo` fallback; it predates `CanBus` and was the original simulated data source before real CAN integration. Speed wanders through city/suburban/highway/stop phases, RPM derived from gear ratios, automatic gear selection (P/R/N/1-7), fuel consumption, coolant temp, boost pressure (0-20 PSI). `CanBus::simulateTick()` reimplements this same phase logic directly in C++ so it can drive the live `sim` property — see [CAN Bus Integration](#can-bus-integration-syvecs-s7)
...
| `icon_oil_pressure.png`, `icon_check_engine.png`, `icon_battery.png`, `icon_coolant_warn.png`, `icon_low_beam.png`, `icon_high_beam.png`, `icon_axle_lift.png`, `icon_cruise.png`, `icon_abs.png` | ISO 7000-style dashboard warning icons (white on transparent) |
...
- **Frame `0x702`** (Base ID+2): byte 2 = bit-masked DIN0-7. Bit *N* = DIN *N*
  ...
  | DIN3 | `lowBeams` |
  | DIN4 | `highBeams` |
  | DIN5 | *(unassigned — planned for `absWarn`, see below; cruise sourced from Syvecs instead)* |
  | DIN6 | *(unassigned — reserved for Frequency 1)* |
  | DIN7 | `hazard` |

  `CanBus` already exposes an `absWarn` bool property (backs the dash's ABS
  lamp) but nothing decodes it yet — same unwired state as `driveMode` above.
  DIN5 is the planned source: it's the only DIN with no datasheet
  double-booking. **This unit is confirmed MCE18 V4** (2026-09-13), so per the
  manual's FAQ 7.7 it can take 12V directly on any DIN — the ABS module's
  warning-lamp feed can land on DIN5 with no divider/level-shifting needed.
...
- `CameraFeed` supports `ULTIMA_CAM_IMAGE_DIR` (env var) to serve a real photo per feed (`<dir>/<label>.png`, e.g. `cam1.png`) instead of the built-in drawn placeholder card — see "Camera feeds" below.
- `ULTIMA_CAM_FPS_LOG=1` still does something real: `CameraView::updatePaintNode()` (`cameraview.cpp`) logs its own texture-upload timing regardless of what's behind the feed. `CameraFeed` itself no longer reads this flag (there's no per-frame decode cost left to log).
- `ULTIMA_CAM_FANOUT=1` (`main.cpp`) still points `cameraFeed2..4` at `cameraFeed1`'s object, so every camera surface shows `cameraFeed1`'s placeholder — a leftover from when this was a render-load proxy for a single real camera; with placeholder feeds there's no load to proxy, it's just a quick way to see all 4 surfaces filled from one feed.

### Camera feeds (placeholder, pending a real driver — 2026-09-01)

No camera driver is wired up (see `beagley-ai/NOTES.md`) — `CameraFeed` (`camerafeed.h`/`.cpp`) no longer touches V4L2 at all. `setActive(true)` just shows a placeholder image immediately: `ULTIMA_CAM_IMAGE_DIR/<label>.png` if that env var is set and the file decodes, otherwise a plain drawn card labeled with the feed's name (`cam1`..`cam4`). `streaming`/`failed`/`frameWidth`/`frameHeight`/`currentFrame()`/`frameReady()` all still work the same as before from `CameraView`/`SurroundView`'s side — only what backs them changed. The zero-copy accessors (`zeroCopy()`, `dmabufFd()`, etc.) are kept as inert stubs (`zeroCopy()` always `false`) purely so `CameraView`/`SurroundView`/`DmaBufTextureSet` didn't need to change — they already fall back to the `currentFrame()` QImage path when zero-copy isn't available.

The two sections below (`Camera frame paths`, `Camera framerate: root cause found`) document the real V4L2/zero-copy/NEON pipeline that existed before this — deleted along with the rest of the mycam004m-driver-specific code, since none of it can run without a driver anyway and it was written against that driver's specific format/buffer contract. Kept as reference for when a new driver exists and this gets rebuilt for real; none of it describes current code.

### Camera frame paths (zero-copy vs converted) — historical, pre-driver-removal
...
### Camera framerate: root cause found (4 fps → 25 fps) — historical, pre-driver-removal
