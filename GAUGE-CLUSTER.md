...
- **`bluetoothmanager.h` / `bluetoothmanager.cpp`** — `BluetoothManager` backs the
  ...
  **Neither iOS nor Android's built-in Bluetooth settings actually pairs with a bare
  ...
  is still used for pairing confirmation (adapter-level, not role-specific). Both are
  constructed lazily (only when the pairing screen is actually opened, via
  `startAdvertising()`) rather than at app startup — merely constructing them was
  enough to trigger a CoreBluetooth permission prompt on the macOS dev build.
...
- **`BluetoothScreen.qml`** — Bluetooth pairing overlay, opened by the bottom-right glyph on `main.qml`. Calls into `BluetoothManager` (the `bluetooth` context property) to start advertising the dash (as `"Ultima RS"`) on open; shows "Discoverable as ..." status, the currently-connected device (if any) or a "Waiting for a connection..." state, and a pairing-confirmation panel (code display + accept/reject, or just a wait state for "Just Works"/PIN-display pairing) when `bluetooth.pendingPairAddress` is set. No scan/device list — a phone connects *to* the dash, not the other way around, but not via the phone's own Bluetooth settings (see the `bluetoothmanager.h`/`.cpp` bullet above — that path doesn't work on current iOS or Android). Same overlay styling as `SetTimeScreen` (full-screen black, bahnschrift/range fonts, `DashButton`-style controls)
